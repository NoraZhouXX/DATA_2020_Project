...
LVAR <- function(y, p = 1, init = c("OLS", "LASSO", "ELASSO")) {
  ...
  # OLS
  if (init == "OLS") {
    datamatOLS <- as.data.frame(rhs)
    for (i in 1:K) {
      yi <- yend[, i]
      equation[[colnames(yend)[i]]] <- lm(yi ~ . - 1, data = datamatOLS)
    }
  }
  ...
  if (init == "LASSO") {
    for (i in 1:K) {
      yi <- yend[, i]
      set.seed(1)
      cv.out  <- cv.glmnet(datamat, yi, family = "gaussian", alpha = 1,
                           intercept = FALSE)
